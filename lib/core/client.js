...
class NightwatchAPI {
  ...
  __isBrowserName(browser, alternateName) {
    const lowerCaseBrowserName = this.browserName.toLowerCase();
...
class NightwatchClient extends EventEmitter {
  ...
  get initialCapabilities() {
    if (isObject(this.settings.capabilities) && JSON.stringify(this.settings.capabilities) !== '{}') {
  ...
  createSession({argv, moduleKey = ''} = {}) {
    ...
    return this.transport.createSession({argv, moduleKey})
      .then(data => {
