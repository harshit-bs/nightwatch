...
const NightwatchEventHandler = require('../eventHandler.js');
...
class CliRunner {
  ...
  eventSetup() {
    if (this.isOnEventExists()) {
      this.test_settings.eventHandler = new NightwatchEventHandler(this.customisedOnEvent.bind(this));
    }
  }
  ...
  runGlobalHook(key, args = [], isParallelHook = false) {
    let promise;
    const {eventHandler, globals} = this.test_settings;

    if (eventHandler && Utils.isFunction(globals[key])) {
      eventHandler.emit({
