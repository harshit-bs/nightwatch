...
const {NightwatchEventHub, USER_EVENTS} = require('../eventHub.js');
const {GlobalHookRunStarted, GlobalHookRunFinished} = USER_EVENTS;
...
class CliRunner {
  ...
  customisedOnEvent(...args) {
    const {globals} = this.test_settings;
    const {plugins} = this.globals;

    if (Utils.isFunction(globals.onEvent)) {
      globals.onEvent(...args);
    }

    if (plugins.length > 0) {
      plugins.forEach((plugin) => {
        if (plugin.globals && Utils.isFunction(plugin.globals.onEvent)){
          plugin.globals.onEvent(...args);
        }
      });
    }
  }

  isOnEventCallbackExistsInGlobal() {
    const {globals} = this.test_settings;
    const {plugins} = this.globals;
  
    return Utils.isFunction(globals.onEvent) || plugins.some(plugin => plugin.globals && Utils.isFunction(plugin.globals.onEvent));
  }

  eventHubSetup() {
    if (this.isOnEventCallbackExistsInGlobal()) {
      this.test_settings.eventHub = new NightwatchEventHub(this.customisedOnEvent.bind(this));
    }
  }
  ...
  runGlobalHook(key, args = [], isParallelHook = false) {
    let promise;
    const {eventHub, globals} = this.test_settings;

    if (eventHub && Utils.isFunction(globals[key])) {
      eventHub.emit({
        eventName: GlobalHookRunStarted,
        hook_type: key
      });
    }
    ...
    return promise.finally(() => {
      if (eventHub && Utils.isFunction(globals[key])) {
        eventHub.emit({
          eventName: GlobalHookRunFinished,
          hook_type: key
        });
      }
    });
