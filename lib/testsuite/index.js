...
const {USER_EVENTS} = require('../runner/eventHub');
const {GlobalHookRunStarted, GlobalHookRunFinished, TestSuiteStarted, TestSuiteFinished, HookRunStarted, HookRunFinished, TestRunStarted, TestRunFinished} = USER_EVENTS;
...
class TestSuite {
  ...
  runHook(hookName) {
    ...
    const {eventHub} = this.settings;

    if (eventHub && this.context.hasHook(hookName)) {
      eventHub.emit({
        eventName: HookRunStarted,
  ...
  runGlobalHook(hookName) {
    const {eventHub, globals} = this.settings;

    if (eventHub && globals[hookName]) {
      eventHub.emit({
