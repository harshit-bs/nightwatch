...
class TestSuite {
  ...
  async initClient({initialize = true} = {}) {
    ...
    if (this.validateNightwatchInspectorCriteria()) {
      this.inspectorServer = new NightwatchInspectorServer(this.client);
    }
  }
  ...
  async createContext({context = null, reloadModuleCache = false, suiteTitle = null, attributes = {}} = {}) {
    ...
    if (settings.thitag_filter && settings.tag_filter.length > 0) {
  ...
  runHook(hookName) {
    ...
    const {eventHandler} = this.settings;

    if (eventHandler && this.context.hasHook(hookName)) {
      eventHandler.emit({
        eventName: eventHandler.userEvents.HookRunStarted,
        testResults: this.reporter.testResults.export,
        hook_type: hookName
      });
    }
    ...
    return this.handleRunnable(hookName, () => {
      ...
      if (eventHandler && this.context.hasHook(hookName)) {
        eventHandler.emit({
          eventName: eventHandler.userEvents.HookRunFinished,
          testResults: this.reporter.testResults.export,
          hook_type: hookName
        });
      }
  ...
  runGlobalHook(hookName) {
    const {eventHandler, globals} = this.settings;

    if (eventHandler && globals[hookName]) {
      eventHandler.emit({
        eventName: eventHandler.userEvents.GlobalHookRunStarted,
        hook_type: hookName,
        testResults: this.reporter.testResults.export
      });
    }
    ...
    return this.handleRunnable(hookName, async () => {
      ...
      if (eventHandler && globals[hookName]) {
        eventHandler.emit({
          eventName: eventHandler.userEvents.GlobalHookRunFinished,
          hook_type: hookName,
          testResults: this.reporter.testResults.export
        });
      }
  ...
  async startTestSuite() {
    ...
    if (eventHandler) {
      eventHandler.emit({
        eventName: eventHandler.userEvents.TestSuiteStarted,
        testResults: this.reporter.testResults.export
      });
    }
  ...
  async testSuiteFinished(failures) {
    ...
    if (eventHandler) {
      eventHandler.emit({
        eventName: eventHandler.userEvents.TestSuiteFinished,
        testResults: this.reporter.testResults.export
      });
    }
  ...
  runCurrentTest(testName) {
    ...
    return this.createSession()
      ...
      .then(_ => {
        ...
        if (eventHandler) {
          eventHandler.emit({
            eventName: eventHandler.userEvents.TestRunStarted,
            testResults: this.reporter.testResults.export
          });
        }
      ...
      .then(possibleError => {
        ...
        if (eventHandler) {
          eventHandler.emit({
            eventName: eventHandler.userEvents.TestRunFinished,
            testResults: this.reporter.testResults.export
          });
        }
