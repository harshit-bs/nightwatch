...
class Transport extends BaseTransport {
  ...
  handleConnectError(err) {
    ...
    if (!err.detailedErr && this.driverService) {
      ...
      const detailedErr = `Verify if ${this.serviceName} is configured correctly; using:\n  ${this.driverService.getSettingsFormatted()}\n`;
      const extraDetail = (logPath ? `More info might be available in the log file: ${logPath}` : `\n  Set webdriver.log_path in your Nightwatch config to retrieve more logs from ${this.serviceName}.`);

      if (err.message.includes('Failed to run adb command')) {
        err.help =  [
          `Run "${colors.green('adb start-server')}"`,
          `If still doesn't work, run "${colors.green('npx @nightwatch/mobile-helper android')}"`,
          detailedErr.replace(/^\s{2}/gm, '\t'),
          extraDetail + '\n'
        ];
